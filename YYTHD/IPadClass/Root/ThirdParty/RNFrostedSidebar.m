...
- (void)layoutSubviews {
    ...
    self.imageView.frame = CGRectMake(0, 0, 68, 68);
...
//    NSArray *imageArray = [[NSArray alloc] initWithObjects:@"DOWNLOAD_SEL",
//                           @"MY_COLLECTMV_SEL",
//                           @"MY_COLLECT_LIST_SEL",
//                           @"MY_PLAYLIST_SEL",
//                           @"MY_ORDERAERIST_SEL",
...
//    if ([touch.view isKindOfClass:[RNCalloutItemView class]]) {
//        
//        RNCalloutItemView *calloutView = (RNCalloutItemView *)touch.view;
//        calloutView.imageView.image = IMAGE([imageArray objectAtIndex:calloutView.tag]);
//    }
...
@property (nonatomic, strong) UIImageView *blurLenView;
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            if (idx == (_images.count - 1) && [ShareApp.unReadCount integerValue] > 0) {
                UIImageView *unImageView = [[UIImageView alloc] initWithFrame:CGRectMake(40, 10, 25, 26)];
                unImageView.image = [UIImage imageNamed:@"OrderUpdate"];
                [view addSubview:unImageView];
                UILabel *unReadLabel = [[UILabel alloc] initWithFrame:CGRectMake(5, 3, 15, 15)];
                unReadLabel.text = [NSString stringWithFormat:@"%@",ShareApp.unReadCount];;
                unReadLabel.textAlignment = NSTextAlignmentCenter;
                unReadLabel.textColor = [UIColor whiteColor];
                unReadLabel.font = [UIFont systemFontOfSize:11];
                unReadLabel.backgroundColor = [UIColor clearColor];
                [unImageView addSubview:unReadLabel];
            }
...
- (void)animateSpringWithView:(RNCalloutItemView *)view idx:(NSUInteger)idx initDelay:(CGFloat)initDelay {
    NSArray *imageArray = [[NSArray alloc] initWithObjects:@"Text_DOWNLOAD",
                           @"Text_MY_COLLECTMV",
                           @"Text_MY_COLLECT_LIST",
                           @"Text_MY_PLAYLIST",
                           @"Text_MY_ORDERAERIST",
                           nil];
    [UIView animateWithDuration:0.2
                     ...
                     completion:^(BOOL finished) {
                         ...
                         UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[imageArray objectAtIndex:idx]]];
                         imageView.frame = CGRectMake(50, 100*idx +140 + idx*10 +5 + idx*2, 50, 9);
         ...
         usingSpringWithDamping:10
          initialSpringVelocity:50
                     ...
                     animations:^{
                         ...
                         view.alpha = 1;
                         UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[imageArray objectAtIndex:idx]]];
                         imageView.frame = CGRectMake(50, 100*idx +170 + idx*10 +5 + idx, 50, 9);
                         imageView.centerY = view.centerY+50;
                         [_contentView addSubview:imageView];
                     }
                     completion:nil];
...
- (void)showInViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    blurImage = [blurImage applyBlurWithRadius:5 tintColor:[UIColor colorWithWhite:0 alpha:0.8] saturationDeltaFactor:1.8 maskImage:nil];
...
//    CGRect contentFrame = self.view.bounds;
    ...
    CGRect contentFrame = CGRectMake(0,0, self.view.height, self.view.height);
    ...
    CGRect blurFrame = CGRectMake(_showFromRight ? self.view.bounds.size.width : 0, 0, 0,  self.view.height);
...
//    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(_showFromRight ? self.view.bounds.size.width : 0, -(59 + topEdge), 0,  self.view.height - 60 - 59 - topEdge)];
//    view.backgroundColor = [UIColor clearColor];
//    [view addSubview:self.blurLenView];
...
- (void)handleTap:(UITapGestureRecognizer *)recognizer {
    ...
    if (! CGRectContainsPoint(self.contentView.frame, location)) {
        [self dismissAnimated:YES];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"setSideButtonSelected" object:nil];
    }
    else {
...
- (void)didTapItemAtIndex:(NSUInteger)index {
    ...
    NSArray *textImageArray = [[NSArray alloc] initWithObjects:@"Text_DOWNLOAD_SEL",
                               @"Text_MY_COLLECTMV_SEL",
                               @"Text_MY_COLLECT_LIST_SEL",
                               @"Text_MY_PLAYLIST_SEL",
                               @"Text_MY_ORDERAERIST_SEL",
                               nil];
    NSArray *imageArray = [[NSArray alloc] initWithObjects:@"DOWNLOAD_SEL",
                           @"MY_COLLECTMV_SEL",
                           @"MY_COLLECT_LIST_SEL",
    ...
    RNCalloutItemView *calloutView = self.itemViews[index];
    calloutView.imageView.image = IMAGE([imageArray objectAtIndex:index]);
    if (self.borderColors) {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[textImageArray objectAtIndex:index]]];
        imageView.frame = CGRectMake(50, 100*index +140 + index*10 +5 + index*2, 50, 9);
        imageView.centerY = view.centerY+50;
        [_contentView addSubview:imageView];
...
- (void)layoutSubviews {
    ...
    self.contentView.frame = CGRectMake(x, 59+topEdge, _width, self.parentViewController.view.bounds.size.height);
